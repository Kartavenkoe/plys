...
template <typename T> // Шаблонный класс для однонаправленного списка
...
template <typename T> // Шаблонный класс для двунаправленного списка
...
template <typename T> // Шаблонный класс для списка уникальных элементов
class UniqueList {
private:
...
void solveTask5() { 
    ...
    while (!(cin >> n) || n <= 0) { // Пока ввод некорректен
        cout << "Некорректное количество абитуриентов. Попробуйте снова: "; 
        cin.clear(); // Очистка состояния потока
        cin.ignore(numeric_limits<streamsize>::max(), '\n'); // Игнорирование оставшихся символов
    }
    cin.ignore(); 
    ...
    for (int i = 0; i < n; ++i) { // Для каждого абитуриента
        ...
        if (!(ss >> lastName >> firstName >> score1 >> score2 >> score3) || // Если ввод некорректен
            score1 < 0 || score2 < 0 || score3 < 0) { // Или оценки отрицательные
            cout << "Некорректные данные абитуриента. Попробуйте снова: ";
            --i; // Уменьшаем счетчик
            continue; // Переходим к следующей итерации
        }

        string fullName = lastName + " " + firstName; // Полное имя
        applicants.add(fullName, make_tuple(score1, score2, score3)); // Добавляем абитуриента в словарь
    }
    ...
    for (const auto& [name, scores] : applicants.getData()) { // Для каждого абитуриента
        if (get<0>(scores) >= 30 && get<1>(scores) >= 30 && get<2>(scores) >= 30 && // Если все оценки не менее 30
            get<0>(scores) + get<1>(scores) + get<2>(scores) >= 140) { // И сумма оценок не менее 140
            admitted.push_back(name); // Добавляем абитуриента в вектор
        }
    }
    ...
    for (const string& name : admitted) { // Для каждого абитуриента
        cout << name << endl; // Выводим имя
    }
}
